...
(() => {
  ...
  const uiObserver = new MutationObserver(() => ensureClockTools());
  uiObserver.observe(document.documentElement, {childList:true,subtree:true});

  const clockScreen = document.getElementById('clockScreen');
  if(clockScreen){
    const screenObserver = new MutationObserver(() => handleClockVisibility());
    screenObserver.observe(clockScreen,{attributes:true,attributeFilter:['class']});
  }
